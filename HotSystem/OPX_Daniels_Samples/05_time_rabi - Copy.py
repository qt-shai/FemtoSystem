...
t_max = 1500 // 4  # in clock cycles units
...
with program() as time_rabi:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(t, t_min, t <= t_max, t + dt):
            ...
            # wait(200//4, 'NV')
            # align('AOM','NV') # only as an example: all that here will wait for the previous to end
            align()
            wait(((300 + 100)// 4), 'AOM')
            wait((300 + 100 + 50) // 4, 'SPCM_OPD')
            play("laser_ON", "AOM")  # 5 microseconds
            measure("readout", "SPCM_OPD", None, time_tagging.digital(times, meas_len, counts))  # 500 ns
            save(counts, counts_st)  # save counts
            align()
            wait((2000-(300+100))//4 - t)  # verify if wait for all channels

        save(n, n_st)  # save number of iteration inside for_loop

    with stream_processing(): # slightly slower (in OPX computer, SOM)
