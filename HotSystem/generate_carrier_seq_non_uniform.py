...
#
# Usage examples:
#   python generate_carrier_seq_non_uniform.py                    # interactive zero-carrier pick
#   python generate_carrier_seq_non_uniform.py --idx 3            # zero-carrier index 3
#   python generate_carrier_seq_non_uniform.py --source corr      # interactive correction BMP pick
#   python generate_carrier_seq_non_uniform.py --source corr --idx 2
#   python generate_carrier_seq_non_uniform.py --source corr --corr "Q:\...path...\MY_CORR.bmp"
#   python generate_carrier_seq_non_uniform.py --force
#
# Non-uniform examples:
#   python generate_carrier_seq_non_uniform.py --xmesh "-300:-260:10,-260:-120:20,-120:50:40"
#   python generate_carrier_seq_non_uniform.py --profile parabola
#
# Override uniform mesh if desired:
#   python generate_carrier_seq_non_uniform.py --x0 -300 --x1 300 --xs 30 --y0 -300 --y1 300 --ys 30
#
# Important Examples:
# python generate_carrier_seq_non_uniform.py --idx 5 --profile uniform --xmesh="-250:-200:5,-200:-150:15,-150:-50:25,-50:50:100,50:250:100" --ymesh="-250:50:150,150:250:10"
import os
import sys
import argparse
import glob
...
# Default UNIFORM mesh (inclusive), periods across the aperture
DEFAULT_X0 = -300
DEFAULT_X1 = 300
DEFAULT_XS = 100
...
# Built-in non-uniform profile intended to compensate an inverted-parabolic
# illumination like in the attached figure: more carriers toward the edges,
# fewer in the bright center region.
PARABOLA_X_SPEC = "-300:-260:10,-260:-180:20,-180:-60:40,-60:-10:20,-10:50:10"
PARABOLA_Y_SPEC = "-100:-70:10,-70:-30:20,-30:30:40,30:70:20,70:100:10"
# --------------------------------
...
def parse_mesh_spec(spec, name):
    """
    Parse a non-uniform mesh specification string.

    Syntax:
        "a:b:s, c, d:e:t, ..."

    where "a:b:s" expands to [a, a+s, ..., b] (inclusive, integer),
    and single numbers are used as-is. Duplicates are removed while
    preserving order.
    """
    if not spec:
    ...
    for part in spec.split(","):
        ...
        if ":" in part:
            bits = part.split(":")
            if len(bits) != 3:
                raise ValueError(f"Bad range token in {name}: '{part}' (expected a:b:s)")
            a, b, s = map(int, bits)
            if s == 0:
                raise ValueError(f"Zero step in {name} token '{part}'")
            sign = 1 if s > 0 else -1
            rng = list(range(a, b + sign, s))
            values.extend(rng)
        else:
    ...
    for v in values:
        if v not in seen:
            ...
            out.append(v)
    if not out:
        raise ValueError(f"No values parsed from {name} spec '{spec}'")
    return out
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate SLM BMP sequence by adding carriers to a source phase (zero-carrier or correction BMP)."
    )
    ...
    parser.add_argument("--idx", type=int, default=None,
                        help="Index for the chosen source (1-based; shown in list).")
    parser.add_argument("--corr", type=str, default=None,
                        help="Full path to a specific correction BMP (overrides --idx for corr).")
    parser.add_argument("--force", action="store_true",
                        help="(Kept for backward compatibility; now all BMPs in OUT_DIR are cleared automatically.)")
    ...
    # ---- NEW: non-uniform mesh options ----
    parser.add_argument(
        "--xmesh", type=str, default=None,
        help=("Non-uniform mesh spec for Cx (overrides x0/x1/xs). "
              "Format: 'a:b:s, c, d:e:t,...' where 'a:b:s' expands to "
              "[a, a+s, ..., b] (inclusive).")
    )
    parser.add_argument(
        "--ymesh", type=str, default=None,
        help=("Non-uniform mesh spec for Cy (overrides y0/y1/ys). "
              "Same format as --xmesh.")
    )

    parser.add_argument(
        "--profile", choices=["uniform", "parabola"], default="uniform",
        help=("High-level mesh profile. 'uniform' uses regular grids "
              "(possibly overridden by x0/x1/xs etc.). "
              "'parabola' uses a built-in non-uniform mesh meant to "
              "flatten an inverted-parabolic illumination like the "
              "figure you provided. If --xmesh/--ymesh are given, they "
    ...
    # ----- Build carrier grids (inclusive) -----
    # Priority: explicit xmesh/ymesh > profile > uniform defaults
    if args.xmesh is not None:
        xs = parse_mesh_spec(args.xmesh, "xmesh")
    elif args.profile == "parabola":
        xs = parse_mesh_spec(PARABOLA_X_SPEC, "PARABOLA_X_SPEC")
    else:
        xs = generate_mesh(args.x0, args.x1, args.xs)

    if args.ymesh is not None:
        ys = parse_mesh_spec(args.ymesh, "ymesh")
    elif args.profile == "parabola":
        ys = parse_mesh_spec(PARABOLA_Y_SPEC, "PARABOLA_Y_SPEC")
    else:
    ...
    print(f"Generating {total} frames into {OUT_DIR} "
          f"(X mesh size={len(xs)}; Y mesh size={len(ys)})")
    if (0 in xs) and (0 in ys):
        print("Note: skipping (Cx=0, Cy=0) frame.")

    order_str = "SHUFFLED" + (f" (seed={args.seed})" if args.seed is not None else "")
    if not shuffled:
        order_str = "RASTER"
    ...
    manifest = {
        ...
        "out_dir": OUT_DIR,
        "x_mesh": {
            "start": args.x0, "stop": args.x1, "step": args.xs,
            "values": xs, "spec": args.xmesh, "profile": args.profile
        },
        "y_mesh": {
            "start": args.y0, "stop": args.y1, "step": args.ys,
            "values": ys, "spec": args.ymesh, "profile": args.profile
        },
