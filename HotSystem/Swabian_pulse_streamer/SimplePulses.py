...
#python_pulseStreamer module for scientific computing only used for creating the random pulse and merging signals
import numpy as np
...
ip_hostname= '192.168.101.64'#'pulsestreamer' # edit this line to use a specific Pulse Streamer IP address

pulser = PulseStreamer(ip_hostname)

#define channel names
ch_timetagger_sync = 0 # output channel 0
ch_highland0050    = 1 # output channel 0
ch_highland0062    = 2 # output channel 1
ch_spcm_gate       = 3 # output channel 3
ch_red_aom       = 4 # output channel 4
# gate_low      = 13 #ns
gate_low      = 0 #ns
trigger_high  = 50 #ns
delay_spcm    = 92 #ns
...
seq_timetagger_sync = [(trigger_high, HIGH), (seqPeriod - trigger_high, LOW)]
seq_highland0050 = [(trigger_high, HIGH), (seqPeriod - trigger_high, LOW)]
seq_highland0062 = [(trigger_high, HIGH), (seqPeriod - trigger_high, LOW)]
seq_spcm_gate = [(delay_spcm, HIGH), (gate_low, LOW), (seqPeriod - gate_low - delay_spcm, HIGH)]
# seq_spcm_gate = [(seqPeriod, HIGH)] # constant open
seq_red_aom = [(delay_aom, LOW), (aom_open, HIGH), (seqPeriod - aom_open - delay_aom, LOW)]
# seq_red_aom = [(seqPeriod, HIGH)] # constant open
...
seq.setDigital(ch_timetagger_sync, seq_timetagger_sync)
seq.setDigital(ch_highland0050, seq_highland0050)
seq.setDigital(ch_highland0062, seq_highland0062)
seq.setDigital(ch_spcm_gate , seq_spcm_gate)
seq.setDigital(ch_red_aom , seq_red_aom)
