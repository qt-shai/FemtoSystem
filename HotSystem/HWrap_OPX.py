...
class GUI_OPX():
    ...
    def UpdateWait_time_benchmark(sender, app_data, user_data):
        sender.Wait_time_benchmark = (int(user_data))
        time.sleep(0.001)
        dpg.set_value(item="inWait_time", value=sender.Wait_time_benchmark)
        print("Set Wait_time to: " + str(sender.Wait_time_benchmark))
    ...
    def GUI_ParametersControl(self, isStart):
        ...
        if isStart:
            ...
            with dpg.collapsing_header(label="Parameter Controls", tag="Parameter_Controls_Header", parent="Params_Controls", default_open=False):
                ...
                dpg.add_text(default_value="Wait time [s]", parent="Benchmark_group", tag="Wait_time_benchmark", indent=-1)
                dpg.add_input_int(label="", tag="inWait_time", indent=-1, parent="Benchmark_group",
                                  width=item_width, callback=self.UpdateWait_time_benchmark,
                                  default_value=self.Wait_time_benchmark, min_value=0, max_value=100, step=1)
    ...
    def QUA_execute(self, closeQM = False, quaPGM = None,QuaCFG = None):
        ...
        if self.bEnableSimulate:
            ...
            sourceFile.close()
            simulation_config = SimulationConfig(duration=28000*3)  # clock cycles
    ...
    def benchmark_play_list_of_gates(self,N_vec,n,idx):
    ...
    def Random_Benchmark_QUA_PGM(self):
        ...
        #
        #         self.QUA_shuffle(N_idx_vec_qua, self.n_measure)  # shuffle - idx_vec_qua vector is after shuffle
        #
        #         # sequence
        #         with for_(idx, 0, idx < self.n_measure, idx + 1):
        ...
        #
        #                 align("MW","RF")
        #                 self.benchmark_play_list_of_gates(N_idx_vec_qua,n, idx)
        #                 wait(t_wait)
        #
        #                 # play Laser
        #                 align("RF", "Laser")
        ...
        #                 assign(counts[N_idx_vec_qua[idx]], counts[N_idx_vec_qua[idx]] + counts_tmp)
        #                 align()
        #
        #             with else_():
        ...
        #         # stream
        #         with if_(sequenceState == 0):
        #             with for_(jdx, 0, idx < self.n_measure,
        #                       jdx + 1):  # in shuffle all elements need to be saved later to send to the stream
        ...
        #         counts_st.buffer(len(self.N_vec)).average().save("counts")
        #         counts_ref_st.buffer(len(self.N_vec)).average().save("counts_ref")
        #         n_st.save("iteration")
        ...
        # time scan vector
        tScan_min = 1  # in [cycles]
        tScan_max = self.n_measure  # in [cycles]
        t_wait = self.time_in_multiples_cycle_time(self.Twait) //4
        dt = 10  # in [cycles]
        self.t_vec = [i * 1 for i in range(tScan_min, tScan_max + 1, dt)]  # in [nsec], used to plot the graph
        self.t_vec_ini = np.arange(tScan_min, tScan_max + dt, dt)  # in [cycles]
            ...
            with for_(n, 0, n < self.n_avg, n + 1):
                ...
                with for_(idx, 0, idx < array_length, idx + 1):
                    ...
                    with if_(sequenceState == 0):
                        ...
                        align("MW", "RF")
                        self.benchmark_play_list_of_gates(idx_vec_qua,n, idx)
                        wait(t_wait)
                        # play Laser
                        align("RF", "Laser")
                        play("Turn_ON", "Laser", duration=tSettle // 4)
    ...
    def btnStop(self):  # Stop Exp
        try:
            ...
            if self.exp not in [Experiment.COUNTER, Experiment.SCAN, Experiment.RandomBenchmark]:
                self.btnSave()
        except Exception as e:
